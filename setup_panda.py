...
def run_config(name):
    ...
    os.system("(cd build.foc_panda;"+command+")")
def prepare_build():
    print "Setup build"
    command = sys.argv[2]+"/tool/create_builddir foc_panda BUILD_DIR=build.foc_panda"
    print "Execute: ", command
    os.system(command)
    f = open('build.foc_panda/etc/build.conf', 'a')
